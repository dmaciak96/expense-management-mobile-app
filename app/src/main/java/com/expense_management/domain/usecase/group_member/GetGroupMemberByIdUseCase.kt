...
class GetGroupMemberByIdUseCase @Inject constructor(private val repository: GroupMemberRepository) {
        ...
        repository.getById(id)
            .map { result ->
                when (result) {
                    is Success -> {
                        if (result.data == null) {
                            return@map Error(ElementNotFoundException("Group member not found with id:$id"))
                        }
