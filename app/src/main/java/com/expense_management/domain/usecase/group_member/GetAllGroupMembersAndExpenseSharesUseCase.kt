...
class GetAllGroupMembersAndExpenseSharesUseCase @Inject constructor(private val repository: GroupMemberRepository) {
    operator fun invoke() =
        repository.getGroupMemberAndExpenseShares()
            .map { result ->
                when (result) {
