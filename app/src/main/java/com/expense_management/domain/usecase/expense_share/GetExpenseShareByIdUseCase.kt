...
class GetExpenseShareByIdUseCase @Inject constructor(private val repository: ExpenseShareRepository) {
        ...
        repository.getById(id)
            .map { result ->
                when (result) {
                    is Success -> {
                        if (result.data == null) {
                            return@map Error(ElementNotFoundException("Expense share not found with id:$id"))
                        }
